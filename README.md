...
Wrangler publishes Workers Static Assets with no server executor, database, or model credentials. It binds only `kujo.robertdevore.com` in the existing zone, disables workers.dev and preview URLs, and serves directory-index routes plus a real 404. The build includes the read-only Kujo SSG WebMCP v1 public index and browser adapter. Progress stays in the browser. Authored HTML is trusted build input; this is not a publishing service for arbitrary untrusted Markdown.

The social card is generated by Howl from `howl.json`; see `showcase/README.md` for regeneration and renderer requirements. Departure Mono headings, UI labels, and code are self-hosted; the header/footer/favicon use the official Kujo SVG mark. The reviewed PNG is committed so deployment does not need a raster renderer.

The full SEO/AI-search audit lives in `seo-audit/2026-09-07/`, including immutable baseline output, crawls, Lighthouse receipts, issues, and measurement limits. `scripts/seo-audit.mjs` and `scripts/seo-edge.mjs` reproduce the same inventory/probes for a new audit workspace; never overwrite the sealed baseline.
