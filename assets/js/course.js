(() => {
  'use strict';
  const icon = name => {const el=document.createElement('span');el.className='icon icon-'+name;el.setAttribute('aria-hidden','true');return el;};
  const labelIcon=(el,label,name)=>el.replaceChildren(document.createTextNode(label+' '),icon(name));
  ...
  const renderProgress = () => {
    ...
    buttons.forEach(b => { const done = completed.has(b.dataset.complete); labelIcon(b,done ? 'Completed · mark incomplete' : 'Mark complete',done?'check':'circle'); b.setAttribute('aria-pressed', String(done)); });
    ...
    if(resume && completed.size){const next=[...document.querySelectorAll('.stage [data-lesson]')].find(a=>!completed.has(a.dataset.lesson));if(next){resume.href=next.href;labelIcon(resume,'Continue learning','arrow-right');}else{resume.href='/capstone/';labelIcon(resume,'Continue to the capstone','arrow-right');}}
  };
  ...
  document.querySelectorAll('.prose pre').forEach(pre=>{const code=pre.querySelector('code');if(!code)return;const raw=code.textContent;const language=[...code.classList].find(x=>x.startsWith('language-'))?.slice(9)||'text';const wrapper=document.createElement('div');wrapper.className='code-wrap';pre.before(wrapper);const label=document.createElement('span');label.className='code-label';label.textContent=language;const copy=document.createElement('button');copy.type='button';copy.className='copy-code';labelIcon(copy,'Copy','copy');copy.setAttribute('aria-label',`Copy ${language} code`);copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(code.textContent);labelIcon(copy,'Copied','check');}catch{copy.textContent='Select text to copy';}});wrapper.append(label,copy,pre);if(language==='kujo')highlight(code);});
