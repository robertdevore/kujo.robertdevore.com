...
The lesson verifier uses the official Kujo 1.5.0 macOS x64 release artifact, checked against its published SHA-256 checksum. The GitHub release API reported v1.5.0 published on 2026-09-22. This course was reverified on 2026-09-23 UTC. Some tagged documentation still names an older release or labels shipped additions unreleased; the release artifact, release notes, and executable probes resolve those stale labels.
...
Core references: [language specification](https://github.com/kujolang/kujo/blob/v1.5.0/docs/LANGUAGE_SPEC.md), [stable scope](https://github.com/kujolang/kujo/blob/v1.5.0/docs/V1_SCOPE.md), [standard library](https://github.com/kujolang/kujo/blob/v1.5.0/docs/STANDARD_LIBRARY.md), [AI runtime](https://github.com/kujolang/kujo/blob/v1.5.0/docs/AI_RUNTIME.md), [CLI contracts](https://github.com/kujolang/kujo/blob/v1.5.0/docs/CLI_MACHINE_READABLE_CONTRACTS.md), [parity matrix](https://github.com/kujolang/kujo/blob/v1.5.0/docs/VM_INTERPRETER_PARITY_MATRIX.md), and [security posture](https://github.com/kujolang/kujo/blob/v1.5.0/docs/NATIVE_API_SECURITY_POSTURE.md).

## Known release discrepancies

The repository preserves minimal probes and both runtime results under evidence/runtime-discrepancies.json. Loop-local declaration and loop-variable scope probes now agree. Struct assignment, qualified custom-enum matching, and the combined loop-control probe remain discrepancies; successful course examples avoid those patterns. Earlier receipts are preserved under evidence/history/v1.3.1 and evidence/history/v1.4.0 in the repository.

<table><caption>Verified release boundaries</caption><thead><tr><th>Concept</th><th>Intended contract</th><th>Observed 1.5.0 VM</th><th>Course treatment</th></tr></thead><tbody><tr><td>Repeated loop declarations</td><td>Fresh block scope per iteration</td><td>Fixed: both runtimes print 1 and 2</td><td>Keep tested accumulation patterns explicit</td></tr><tr><td>Loop scope</td><td>Loop variable does not leak</td><td>Fixed: both runtimes reject the post-loop reference</td><td>Explicit contract-versus-release note</td></tr><tr><td>Loop control</td><td>Bounded break/continue behavior</td><td>A combined probe exceeded external deadline</td><td>Avoid that pattern in successful builds</td></tr><tr><td>Struct mutation</td><td>Mutable field update</td><td>Assignment left field unchanged</td><td>Construct and read; label fallback needs</td></tr><tr><td>Qualified custom enum match</td><td>Match declared variant</td><td>Probe reached default on VM, matched on interpreter</td><td>Use verified built-in Result/Option examples</td></tr></tbody></table>
...
The tagged specification and scope still name 1.4.0, and some shipped native APIs retain stale unreleased labels. The official 1.5.0 artifact, release record, tagged implementation, and recorded runtime checks establish the version used here. Language and CLI contract schema versions remain 1.0.0.
